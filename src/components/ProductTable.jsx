...
const ProductTable = (tableData) => {
    ...
    const prodList = tableData.prod.map((val) => {

        return (
            <tr className=' bg-bgCard [&>td]:px-2.5
            *:last:p-1 *:last:text-left border-b border-b-bgApp '>
                <td>{val.name}</td>
                <td className='hidden'>{val.price}</td>
                <td className='hidden'>{val.quantity}</td>
                <td>
                    {isInStock && (val.quantity >= val.threshold) ? <p className='w-20 text-[0.8em] text-primary text-center bg-primarySoft rounded-sm border border-primary'>In Stock</p>
